...
export default function DashboardPage() {
  ...
  const [clientsRisqueEleve, setClientsRisqueEleve] = useState<ClientWithScore[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([clientsData, prospectsData, obligationsData]) => {
        ...
        setClientsRisqueEleve(scored.filter((c) => c.score?.niveau === "ELEVE"));
      })
  ...
  const kpis = [
    { label: "Prospects actifs",      value: prospectsActifs,           colorLeft: "border-l-blue-500",    colorText: "text-blue-600",    icon: "↗" },
    { label: "Clients actifs",        value: clientsActifs,             colorLeft: "border-l-emerald-500", colorText: "text-emerald-600", icon: "⚡" },
  ...
  return (
    <div className="min-h-full bg-slate-50">
      ...
      >
        ...
        <p className="text-blue-100 text-sm mt-0.5">Vue d'ensemble du cabinet</p>
      </div>
      ...
      <div className="px-4 -mt-4 space-y-4 pb-4 md:px-8 md:space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {kpis.map(({ label, value, colorLeft, colorText, icon }) => (
            <div key={label} className={`bg-white rounded-2xl border-l-4 ${colorLeft} p-4 shadow-sm border border-white`}>
              <div className="flex items-start justify-between">
                ...
                <span className={`text-lg ${colorText} opacity-60`}>{icon}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-red-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-red-50 bg-red-50/50">
              ...
              <Link href="/dashboard/cartographie" className="text-xs text-red-500 font-medium hover:text-red-700">
                Voir cartographie →
              </Link>
            </div>
            <div className="divide-y divide-slate-50">
              {loading ? (
                ...
                <div className="p-6 text-center text-sm text-slate-400">Aucun client à risque élevé</div>
              ) : (
                clientsRisqueEleve.slice(0, 5).map((client) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-slate-800 truncate">{client.raisonSociale}</p>
                      <p className="text-xs text-slate-400 font-mono">{client.ref}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {client.score && (
                        <span className="text-xs text-slate-400">{client.score.score}/150</span>
                      )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-amber-50 bg-amber-50/50">
              ...
              <Link href="/dashboard/obligations" className="text-xs text-amber-500 font-medium hover:text-amber-700">
                Voir tout →
              </Link>
            </div>
            <div className="divide-y divide-slate-50">
              {loading ? (
                ...
                <div className="p-6 text-center text-sm text-slate-400">Aucune obligation en retard</div>
              ) : (
                obligationsEnRetard.slice(0, 5).map((o) => (
                  <div key={o.id} className="flex items-center justify-between px-4 py-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-slate-800 truncate">{o.type}</p>
                      {o.dateEcheance && (
                    ...
                    <span className="text-xs font-semibold text-red-600 shrink-0 ml-2">EN RETARD</span>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-emerald-50/30">
            ...
            <Link href="/dashboard/prospects" className="text-xs text-emerald-600 font-medium hover:text-emerald-800">
              Voir Kanban →
            </Link>
          </div>
          <div className="px-4 py-3 flex flex-wrap gap-x-4 gap-y-2">
            {loading ? (
              ...
              ["PRISE_CONTACT", "DECOUVERTE", "OPPORTUNITE", "LAB", "PREPARATION"].map((statut) => {
                const count = prospects.filter((p) => p.statutKanban === statut).length;
                return (
                  <div key={statut} className="flex items-center gap-2">
                    <span className="text-sm text-slate-600">{KANBAN_LABELS[statut]}</span>
